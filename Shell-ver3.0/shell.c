...
static int command(int input, int first, int last, char *cmd_exec)
{
  ...
  if (pid == 0) 
  {
    ...
    else if(execvp(args[0], args)<0 && custf == 0) printf("%s: command not found\n", args[0]);
              exit(0);
  }
  else 
  {
...
void custom(char *ip_buf)
{
	...
	if(ip_buf[++i]!='i' || ip_buf[++i]!='s' || ip_buf[++i]!='t') 
		return;
	...
	for(;i<len;i++)
	{
		if(ip_buf[i]==32)//space
			continue;
		if(ip_buf[i]>=65 && ip_buf[i]<91)
			break; 
		if(ip_buf[i]>=97 && ip_buf[i]<123)
			break;
	}
	...
	//printf("j-i = %d",j-i);
	args = (char*) realloc(args,sizeof(char)*(j-i));
	//printf("extension = %s\n",args);
	
	list_all(args);
}
void list_all(char *str)
{
	FILE *ptr;
	//int file = open("Result", O_CREAT|O_RDWR, S_IRWXU);
	//dup2(file,1);
	system("ls > Result");	
	ptr = fopen("Result","r");
	char line[256];
	...
	while(fgets(line,sizeof(line),ptr)){
		...
		int i,x=0;
		for( i = 0; i<slen;i++){
			//printf("Match %c of list with %c of str\n",line[len-(slen-i)-1],str[i]);
			if(str[i] == line[len-(slen-i)-1])	
				x++;
			else
				break;
		}
	...
	printf("%d files of type %s found.",count,str);
	printf("\n");
	fclose(ptr);
}
void prompt()
